...
function TaskForm(props) {
    ...
    const handleFileUpload = (e) => {
        ...
        // });
        let url = _mainContext.localTaskUpload()
        if(url !== '') {
            addNewM3uLinkByUrl(url)
        }else{
            console.error('上传失败');
        }
    }
...
export default function TaskList(props) {
    ...
    const handleDelete = (value) => {
        let res = _mainContext.localTaskDelete()
        if(res) {
            get_task_list()
        }else{
            handleOpenAlertBar(t('删除失败'))
        }
    }

    const get_task_list = () => {
        let list = _mainContext.localTaskGetList()
        setTaskList(list)
    }
    ...
    const doTaskRightNow = (id) => {
        let res = _mainContext.localTaskDoRightNow()
        if(res) {
            get_task_list()
        }else{
    ...
    const getDownloadBody = (id) => {
        let res = _mainContext.localTaskGetTaskDetail()
        if(res) {
            setOpenDownloadBody(true)
            setDownloadBody({ 'content': res.content, "url": "" })
        }else{
            handleOpenAlertBar(t('操作失败'))
        }
    }
